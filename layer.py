...
class DecoderLayer(nn.Module):
    ...
    def forward(self, input, encoder_output):
        ...
        residual = input.clone()
        # Apply dropout to sub-layer output BEFORE adding residual and normalizing
        self_attention = self.masked_attention(input, input, input)
        ...
        # Apply dropout to sub-layer output BEFORE adding residual and normalizing
        output = self.attention(self_attention, encoder_output, encoder_output)
        output = self.dropout2(output)  # Dropout BEFORE residual connection
        output = self.layer_norm2(output + residual)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        # query: (batch_size, q_seq_len, n_head, d_k)
        # key: (batch_size, seq_len, n_head, d_k)
        ...
        if self.mask is not None:
            # Create dynamic mask for current sequence lengths
            if q_seq_len <= self.mask.size(0) and seq_len <= self.mask.size(1):
        ...
        # attention: (batch_size, n_head, q_seq_len, seq_len)
